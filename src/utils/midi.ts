...
export default class Midi
{
    ...
    public static onNoteOn: null | ((note: MidiNote) => any) = null;
    public static onNoteOff: null | ((note: MidiNote) => any) = null;
    private static isEnabled = false;

    private static _noteOnListener(e: any)
    {
        let index = this.notes.findIndex(n => n.name === e.note.name);
        if (index === -1)
        {
            this.notes.push({
                name: e.note.name,
        ...
        this.onNoteOn && this.onNoteOn(this.notes[index]);
    ...
    private static _noteOffListener(e: any)
    {
        ...
        if (index !== -1)
        {
            ...
            this.onNoteOff && this.onNoteOff(this.notes[index]);
            this.onNotesChanged && this.onNotesChanged(this.notes.slice(0));
        }
    }
